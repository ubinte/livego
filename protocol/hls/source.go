...
import (
	...
	"github.com/ubinte/livego/container/ts"
	"github.com/ubinte/livego/parser"
...
func (source *Source) cut() {
	...
	if source.btswriter == nil {
		...
		source.seq++
		filename := fmt.Sprintf("%s%s/%d.ts", source.TsFilePath, source.info.Key, time.Now().Unix())
