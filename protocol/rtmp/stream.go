...
import (
	...
	"github.com/ubinte/livego/protocol/rtmp/cache"
	"github.com/ubinte/livego/protocol/rtmp/core"
	"github.com/ubinte/livego/protocol/rtmp/rtmprelay"
...
type RtmpStream struct {
	...
	getWriters []av.GetWriter
}

func NewRtmpStream() *RtmpStream {
	ret := &RtmpStream{
		streams:    cmap.New(),
		getWriters: make([]av.GetWriter, 0),
	}
	go ret.CheckAlive()
	return ret
}

func (self *RtmpStream) AddGetWriter(getWriter av.GetWriter) {
	self.getWriters = append(self.getWriters, getWriter)
}
...
func (self *RtmpStream) HandleReader(r av.ReadCloser) {
	...
	var stream *Stream
	i, ok := self.streams.Get(info.Key)
	if stream, ok = i.(*Stream); ok {
		...
		if id != EmptyID && id != info.UID {
			...
			stream.Copy(ns)
			stream = ns
			self.streams.Set(info.Key, ns)
		}
	} else {
		stream = NewStream()
		self.streams.Set(info.Key, stream)
		stream.info = info
	}

	stream.AddReader(r)

	for _, getWriter := range self.getWriters {
		writer := getWriter.GetWriter(r.Info())
		self.HandleWriter(writer)
	}
}
...
func (self *RtmpStream) GetStreams() cmap.ConcurrentMap {
	return self.streams
}

func (self *RtmpStream) GetReaders() map[string]av.ReadCloser {
	readers := make(map[string]av.ReadCloser)

	for item := range self.GetStreams().IterBuffered() {
		if stream, ok := item.Val.(*Stream); ok {
			if stream.GetReader() != nil {
				readers[item.Key] = stream.GetReader()
			}
		}
	}
...
func (self *RtmpStream) GetWriters() map[string][]av.WriteCloser {
	writers := make(map[string][]av.WriteCloser)

	for item := range self.GetStreams().IterBuffered() {
		if stream, ok := item.Val.(*Stream); ok {
			ws := stream.GetWs()
			for wsItem := range ws.IterBuffered() {
				if pw, ok := wsItem.Val.(*PackWriterCloser); ok {
					if pw.GetWriter() != nil {
						writers[item.Key] = append(writers[item.Key], pw.GetWriter())
					}
				}
			}
		}
